...
#define MAX_ANALOG_CHANNELSEX       80      // maximum number of data channels (signals) per analog/force plate device
...
// NokovSDK param types//1帧号，2markerset数量 ，3刚体数量，4骨骼数量，5命名点数量，6未命名点数量，7模拟通道数，8参数
typedef enum MocapDataType
{
	MocapDataFrameNO = 1,
	MocapDataMarkerSetNum,
	MocapDataRigidBodyNum,
	MocapDataSkeletonNum,
	MocapDataLabeledMarkerNum,
	MocapDataOtherMarkerNum,
	MocapDataAnalogChNum,
	MocapDataParam,
} MocapDataType;
...
typedef enum
{
	eRigidBodyChange = 1,		//刚体变化通知
	eSkeletonChange,			//骨骼变化通知
	eDataChange,				//数据变化
	eStartPlay,					//开始播放
	ePausePlay,					//暂停播放
	eFrameRateChange,			//帧率变化
}NotifyType;
...
typedef struct
{
	int iFrame;                                 // host defined frame number
	int nAnalogdatas;                           // of Analogdata channels
	int nSubFrame;								// sub frame number
	float Analogdata[MAX_ANALOG_CHANNELSEX][MAX_ANALOG_SUBFRAMES];      // Analog channel data
	unsigned int Timecode;                      // SMPTE timecode (if available)
	unsigned int TimecodeSubframe;              // timecode sub-frame data
	long long iTimeStamp;                       // FrameGroup timestamp
	short params;                               // host defined parameters
} sFrameOfAnalogChannelData;
